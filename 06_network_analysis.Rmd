...
```{r plots-overall}
all_days_pos_plot <- plot_graph(overall$graph_pos, overall$clusters_pos, "microbetag_analysis/figs/flashweave_scores/overall_pos.svg")
all_days_neg_plot <- plot_graph(overall$graph_neg, overall$clusters_neg, "microbetag_analysis/figs/flashweave_scores/overall_neg.svg")
```
...
TG3_pos_plot <- plot_graph(TG3$graph_pos, TG3$clusters_pos, "microbetag_analysis/figs/flashweave_scores/TG3_pos.svg")
TG3_neg_plot <- plot_graph(TG3$graph_neg, TG3$clusters_neg, "microbetag_analysis/figs/flashweave_scores/TG3_neg.svg")
```
...
TG4_pos_plot <- plot_graph(TG4$graph_pos, TG4$clusters_pos, "microbetag_analysis/figs/flashweave_scores/TG4_pos.svg")
TG4_neg_plot <- plot_graph(TG4$graph_neg, TG4$clusters_neg, "microbetag_analysis/figs/flashweave_scores/TG4_neg.svg")
```
...
```{r}
plot_graph <- function(graph, cluster, min_community_size = 2, vertex_color = NULL, vertex_size = NULL, edge_width = 1, mark_col = "#f4f4f4") {
  igraph::plot.igraph(
    graph,
...
# 
# plots <- list(
#   TG1_neg_plot, TG1_pos_plot, TG2_neg_plot, TG2_pos_plot, TG3_neg_plot, TG3_pos_plot, TG4_neg_plot, TG4_pos_plot, TG5_neg_plot, TG5_pos_plot,
#   d7_neg_plot, d7_pos_plot, d14_neg_plot, d14_pos_plot, d21_neg_plot, d21_pos_plot, d28_neg_plot, d28_pos_plot, d35_neg_plot, d35_pos_plot
# )
# 
# grid_plot <- wrap_plots(plots, ncol = 5)
graphs <- list(
  TG1$graph_neg, TG1$graph_pos, TG2$graph_neg, TG2$graph_pos, TG3$graph_neg, TG3$graph_pos, TG4$graph_neg, TG4$graph_pos, TG5$graph_neg, TG5$graph_pos,
  day7$graph_neg, day7$graph_pos, day14$graph_neg, day14$graph_pos, day21$graph_neg, day21$graph_pos, day28$graph_neg, day28$graph_pos, day35$graph_neg, day35$graph_pos
)

names <- list(
  TG1$clusters_neg, TG1$clusters_pos, TG2$clusters_neg, TG2$clusters_pos, TG3$clusters_neg, TG3$clusters_pos, TG4$clusters_neg, TG4$clusters_pos, TG5$clusters_neg, TG5$clusters_pos,
  day7$clusters_neg, day7$clusters_pos, day14$clusters_neg, day14$clusters_pos, day21$clusters_neg, day21$clusters_pos, day28$clusters_neg, day28$clusters_pos, day35$clusters_neg, day35$clusters_pos
)

# Open a plotting device (PDF or PNG if saving)
pdf("microbetag_analysis/figs/flashweave_scores/4x5_graph_grid.pdf", width = 20, height = 20)

# Set up 5x5 plot layout
par(mfrow = c(4, 5), mar = c(0, 0, 0, 0))  # no margins

# Call plot_graph for each of the 25 cases
for (i in 1:20) {
  plot_graph(graphs[[i]], names[[i]])  # assuming lists of 25 graphs & clusters
}
...
day_clusters_positive <- c(
  day7$communities_pos, 
...
day_clusters_negative <- c(
  day7$communities_neg, 
...
# Summary statistics
distance_stats <- bind_rows(
  associated_pairs_positive %>% mutate(type="positive"),
  ...
  group_by(type) %>% 
  summarise(
    weighted_mean = sum(distance * count) / sum(count),
...
permutation_test <- function(data, n_permutations = 1000) {
  ...
  original_diff <- abs(
    weighted_mean(
      data$distance[data$type == "positive"], data$count[data$type == "positive"]
    ) - weighted_mean(
      data$distance[data$type == "negative"], data$count[data$type == "negative"]
    )
  )
  ...
  perm_diff <- replicate(n_permutations, {
    ...
    abs(
      weighted_mean(
        data$distance[shuffled_type == "positive"], data$count[shuffled_type == "positive"]
      ) - weighted_mean(
        data$distance[shuffled_type == "negative"], data$count[shuffled_type == "negative"]
        )
    )
  })
...
ggsave(
  "microbetag_analysis/figs/pcoa/pcoa.svg", q1, 
  dpi = 300, width = 9,  height = 9, units = "in", device = "svg", bg = "transparent"
) 
...
genomes_sets <- list(
  oscillospirales = Oscillospirales,
  lachnospirales = Lachnospirales,
  entero_christensenellales = c(Enterobacterales, Christensenellales),
  small = c(Erysipelotrichales, UBA1381, TANB77, Clostridiales, RF39)
)


for (set_name in names(genomes_sets)) {
  
  taxa <- genomes_sets[[set_name]]
  genomes_of_interest <- intersect(taxa, connected_genomes)
  ...
  q1 <- ggplot() +
    geom_segment(
      ...
      color="#f5a2af", linewidth=1
    ) +
    geom_segment(
      data = associated_pairs_positive,
      aes(x = x1, y = y1, xend = x2, yend = y2, linetype=clustering),
      color="#6AA84F", linewidth=1
    ) +
    scale_linetype_manual(values=c("solid", "dashed", "dotted")) +
    geom_point(
      data = plot_data,
    ...
    geom_label(
      data = plot_data %>% filter(genome %in% genomes_of_interest),
      aes(x = Axis.1 , y = Axis.2 , label = auto),
      size = 3, fill = "white", color = "black", alpha = 0.31
    ) +
    ...
    scale_x_continuous(expand = expansion(add = c(1.1, 0.3))) +
    scale_y_continuous(expand = expansion(add = c(0.1, 0.1)))
  
  ggsave(
    filename = paste0("microbetag_analysis/figs/pcoa/pcoa_", set_name, ".svg"),
    plot = q1,
    dpi = 300, width = 9, height = 9, units = "in",
...
compl_scores <- read.table("microbetag_analysis/mgg_prec/seeds_complementarity/phylomint_scores.tsv", sep = "\t", header = TRUE)
...
heatmap_phylum <- create_taxonomic_heatmap(phyla_filtered_results, genome_taxonomy, "phylum")
heatmap_order  <- create_taxonomic_heatmap(order_filtered_results, genome_taxonomy, "order")

# Set desired dimensions
svg("microbetag_analysis/figs/seed_stats/seed_scores_lm_order2.svg", width = 8, height = 6, bg="transparent")  
