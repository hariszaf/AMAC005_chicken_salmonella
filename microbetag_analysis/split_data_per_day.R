...
metadata = readxl::read_excel("sample_metadata.xlsx", sheet = "sample_metadata")
...
metabolites = readxl::read_excel("sample_metadata.xlsx", sheet = "mets_of_interest")
...
# Save to file
write.table(all_metadata_t, "all_metadata_mets_transp.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
...
# Load genome counts
genome_count_table <- read.table("salm_multio_abd_table_gc.tsv", sep = "\t", header = TRUE)

day_7_abd <- genome_count_table %>%
  select(any_of(c("genome", day_7_metadata$sample, "classification")))
write.table(day_7_abd, "per_day/abd_day_7.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)

day_14_abd <- genome_count_table %>%
  select(any_of(c("genome", day_14_metadata$sample, "classification")))
write.table(day_14_abd, "per_day/abd_day_14.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)

day_21_abd <- genome_count_table %>%
  select(any_of(c("genome", day_21_metadata$sample, "classification")))
write.table(day_21_abd, "per_day/abd_day_21.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)

day_28_abd <- genome_count_table %>%
  select(any_of(c("genome", day_28_metadata$sample, "classification")))
write.table(day_28_abd, "per_day/abd_day_28.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)

day_35_abd <- genome_count_table %>%
  select(any_of(c("genome", day_35_metadata$sample, "classification")))
write.table(day_35_abd, "per_day/abd_day_35.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)



# Remember FlashWeave only considers .tsv extension for metadata files
write.table(t(select(day_7_metadata,  -c(sample, individual))), "per_day/all_metadata_day_7.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
write.table(t(select(day_14_metadata, -c(sample, individual))), "per_day/all_metadata_day_14.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
write.table(t(select(day_21_metadata, -c(sample, individual))), "per_day/all_metadata_day_21.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
write.table(t(select(day_28_metadata, -c(sample, individual))), "per_day/all_metadata_day_28.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
write.table(t(select(day_35_metadata, -c(sample, individual))), "per_day/all_metadata_day_35.tsv", sep = "\t", quote=FALSE, row.names = TRUE, col.names = FALSE)
